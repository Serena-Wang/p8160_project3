...
Additional plots are presented below to further investigate the model's performance. The graph displaying the average prediction points by month shows that our model performs well across different months, except for January, which has limited data points. When we evaluate our predictions by year or by nature, we see consistent performance across different years and types of hurricanes.
\begin{figure}[H] 
\centering
...
\begin{figure}[H] 
\centering
\includegraphics[width=1.0\textwidth]{Results/NATURE.png} 
\caption{Actual Wind Speed vs.Predicted Wind Speed by Nature}
\label{Figure}
\end{figure}
