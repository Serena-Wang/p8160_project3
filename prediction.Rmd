...
hurricane %>% 
  ...
  group_by(ID) %>% 
  summarise(nobs = n(),
            Pred_SE = sqrt(sum(Pred_E_Sq))) %>% 
  arrange(Pred_SE) %>% 
  top_n(10) %>% 
  knitr::kable(
      caption = "Prediction Standard Error of Wind Speed in Ascending Order", 
      col.names = c("Hurricane ID", "Number of Observation", "Prediction SE"),
      digits = 3
  )
```
...
hurricane %>% 
  ...
  ggplot(aes(x = Season)) +
  geom_point(aes(y = Ave.Speed, color = "Ave.Speed"), alpha = 0.6) +
  geom_point(aes(y = Ave.Pred.Speed, color = "Ave.Pred.Speed"), alpha = 0.6) +
  scale_color_manual(name = "Wind Speed",
                     values = c("Ave.Speed" = "#404080",  
                                "Ave.Pred.Speed" = "#69b3a2")) +
  labs(title = "Figure: Actual Wind Speed V.S. Prediction Wind Speed By Year",
       x = "Year",
       y = "Wind Speed")
```
...
hurricane %>% 
  ...
  scale_color_manual(name = "Wind Speed",
                     values = c("Ave.Speed" = "#404080",  
                                "Ave.Pred.Speed" = "#69b3a2")) +
  labs(title = "Figure: Actual Wind Speed V.S. Prediction Wind Speed By Month",
       x = "Month",
       y = "Wind Speed") +
...
hurricane %>% 
  ...
  scale_color_manual(name = "Wind Speed",
                     values = c("Ave.Speed" = "#404080",  
                                "Ave.Pred.Speed" = "#69b3a2")) +
  labs(title = "Figure: Actual Wind Speed V.S. Prediction Wind Speed By Nature",
       x = "Nature",
       y = "Wind Speed")
```
...
hurricane %>% 
  ...
  scale_color_manual(name = "Wind Speed",
                     values = c("Speed" = "#404080",  
                                "Pred.Speed" = "#69b3a2")) +
  labs(title = "Figure: Actual Wind Speed V.S. Prediction Wind Speed For GINGER.1971",
       x = "Time",
       y = "Wind Speed")
```
...
hurricane %>% 
  ...
  labs(title = "Figure: Prediction Standard Error of Wind Speed by Number of Observation",
       x = "Observation",
