...
$$\begin{aligned} f(B,\mu|\Sigma) =& f(B|\mu, \Sigma)  \times f(\mu)\\
=& det(A)^{H/2}\prod_{i=1}^{H} exp(-\frac{(\beta_i-\mu)^\intercal A (\beta_i-\mu)}{2})\times
det(V)^{\frac{-1}{2}}exp({-\frac{\mu^\intercal V^{-1}\mu}{2}})
\end{aligned}$$  

$$\begin{aligned} f(B|\mu, \sigma^2, \Sigma, \gamma, Y)\propto \prod_{i=1}^{H} exp(-\frac{(\beta_i-\mu)^\intercal A (\beta_i-\mu)}{2})\end{aligned} $$
