...
# B is 5 x H
B_post <- function(D, sigma_squared, A, X_with_id, gamma, mu){
  B <- NULL
  #number of hurricanes
  H <- max(X_with_id[,1])
  total_obs <- nrow(D)
  for (i in 1:H){
    # can map this to a list
    ith_hurricane_idx <- which(X_with_id[,1] == i)
    n_i <- length(ith_hurricane_idx)
    X_i <- X_with_id[ith_hurricane_idx,][,-1]
    Y_i <- Y[ith_hurricane_idx,]
    D_i <- D[ith_hurricane_idx,]
    M_i <- t(D_i)%*% 
