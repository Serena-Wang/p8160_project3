...
X <- data  %>%
  ...
  ungroup() %>%
  mutate(intercept = 1) %>%
  dplyr::select(intercept, April, May, June, July, August, September, October, November, December, Season, ET,NR,SS,TS) %>%
  as.matrix()
r <- ranef(glmm) 
B <-  r$ID %>% as.matrix() %>% t() %>% as.matrix()
gamma <-c (37.841607,gamma_init) %>% as.matrix() 
...
gamma_post <- function(X,Y, D,B, sigma_squared){
  M <- 0
  N <- 0
  for (i in 1:H){
    curr_hurricane_idx <- ith_hurricane_idx[[i]]
...
test_sigma_squared_post <- function(Y, D, B , X, gamma){
  ...
  DB <- D%*%B
  Y_minus_mu <- Y-DB[,1,drop = FALSE]- X%*%gamma
...
hurricane_gibbs_sampling <- function(start_sigma_squared, 
  ...
  for (i in 2:nitr){
    ...
    sigma_squared <- mean(chain[400:500])
...
nov_beta <- rep(NA, 100)
dec_beta <- rep(NA, 100)
season_beta <- rep(NA, 100)
et_beta <- rep(NA, 100)
nr_beta <- rep(NA, 100)
ss_beta <- rep(NA, 100)
ts_beta <- rep(NA, 100)

plot(1:100, type='l', sapply(1:100, function(x) test[["B"]][[x]][1,1]))
```
```{r}
plot(1:100, type='l', sapply(1:100, function(x) test[["gamma"]][[x]][15,1]))
```
