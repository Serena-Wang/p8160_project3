...
for (i in 1:4){
  hist(freq = F, nclass = 100, sapply(1:nitr, function(x) test[["gamma"]][[x]][i]), main= paste0("gamma_", i, "_value"))
}
```
...
par(mfrow=c(2,2),mar = c(2, 5, 1, 1))
for (i in 2:5){
  hist(freq = F, nclass = 100, sapply(1:nitr, function(x) test[["mu"]][[x]][i]), main= paste0("mu_", i-1, "_value"))
}
```
...
for (i in 1:5){
  hist(eval(parse(text = paste0("beta_", i))), freq = F, nclass = 100, main =  paste0("beta_", i, "_value"))
}
```
...
for (r in 1:5){
  for (i in 1:5){
    hist(freq = F, nclass = 100, sapply(1:nitr, function(x) test[["A"]][[x]][r,i]), main= paste0("A_[",r,",", i,"]", "_value"))
  }
}
```
